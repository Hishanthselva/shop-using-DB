...
def add():
    ...
    la_prnumber=Label(b,text='product Number',bg='white',font=('Times New Roman',16)).place(x=20,y=90)
    ...
    la_prdetails=Label(b,text='products',bg='white',font=('Times New Roman',16)).place(x=20,y=240)
    ...
    def process():
        ...
        sql="insert into products(number,name,price,details)values(%s,%s,%s,%s)"
...
def delete():
    global product
    en1=Entry.get(en)
    cursor.execute('delete from products where number={}'.format(en1))
    db.commit()
    clear()
def search():
    en_1=Entry.get(en)
    cursor.execute('select * from products where number={}'.format(en_1))
    v=cursor.fetchall()
    clear()
    for i in v:
        print(i)
